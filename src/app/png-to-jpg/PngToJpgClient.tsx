...
export default function PngToJpgClient() {
    ...
    try {
      for (const file of files) {
        if (file.type !== "image/png") {
          throw new Error(`File ${file.name} is not a PNG file.`);
        }
        ...
        // Use canvas to draw image and export as jpeg
        ...
        // Fill background with white because PNG might have transparency which becomes black in JPG
        ctx.fillStyle = "#FFFFFF";
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        
        ctx.drawImage(imgElement, 0, 0);

        const dataUrl = canvas.toDataURL("image/jpeg", 0.92); // 0.92 is standard quality
  ...
  return (
    <div className="flex flex-col gap-8 items-center">
      ...
      {files.length > 0 && (
        <button
          onClick={() => handleConvertAction(convertToJpg)}
