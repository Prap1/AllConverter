...
export default function JpegToJfifClient() {
    ...
    try {
      for (const file of files) {
        if (file.type !== "image/jpeg" && file.type !== "image/jpg") {
          throw new Error(`File ${file.name} is not a JPEG file.`);
        }
        ...
        link.href = dataUrl;
        link.download = file.name.replace(/\.jpe?g$/i, ".jfif");
  ...
  return (
    <div className="flex flex-col gap-8 items-center">
      ...
      {files.length > 0 && (
        <button
          onClick={() => handleConvertAction(convertToJfif)}
